...
function validateCurrentScreen() {
    switch (currentScreen) {
        case 1:
            // Screen 1 validation - check bank, account number and amount
...
function getBankDisplayName(bankValue) {
    const bankNames = {
        "blinders-vault": "Blinders Vault",
        stackovercash: "StackOverCash",
    };
    return bankNames[bankValue] || bankValue;
}
...
function updateReceiptData() {
    ...
    const txnId = "EXT" + Math.floor(Math.random() * 100000);
...
function submitTransfer() {
    ...
    // API endpoint for external transfer
    const apiEndpoint = "Dragon_Vault/api/transfer/external.php";
    ...
    fetch(apiEndpoint, {
        ...
        body: `bank=${encodeURIComponent(
            bank
        )}&recipient_account=${encodeURIComponent(
            recipient
